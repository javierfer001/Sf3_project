...
class ProductRespository extends EntityRepository {
    ...
    public function getPaginateProduct($pageSize = 3, $currentPage, $id, $type) {
        ...
        //Consulta DQL
        $dql = "SELECT p, pc, c, ps, sb, img FROM MetronicBundle:Product p LEFT JOIN p.categories pc LEFT JOIN pc.category c LEFT JOIN p.subcategories ps LEFT JOIN ps.subCategory sb LEFT JOIN p.imagenes img";
        if ($type == 'cat') {
            $dql .= " WHERE c.id=" . $id;
        } else {
            $dql .= " WHERE sb.id=" . $id;
    ...
    public function productPopulares() {
        //Consulta DQL
        $dql = "SELECT p, pc, c, ps, sb, img, po, popular FROM MetronicBundle:Product p LEFT JOIN p.categories pc LEFT JOIN pc.category c LEFT JOIN p.subcategories ps LEFT JOIN ps.subCategory sb LEFT JOIN p.imagenes img";
        $dql .= "  JOIN p.populars po  JOIN po.popular popular WHERE popular.dateStart <=:date AND popular.active = 1 ";
